        ...
        stage('Run Backend Server') {
            steps {
                ...
                script {
                    try {
                        if (checkOs() == 'Windows') {
                            bat 'start/min python rest_app.py --db_host=$db_host --db_port=$db_port --db_user=$MYSQL_CREDS_USR --db_pass=$MYSQL_CREDS_PSW'
                        } else {
                            //sh 'nohup python rest_app.py &'
                            //sh 'nohup python3 rest_app.py &'
                            sh 'nohup ${python_run_file} rest_app.py --db_host=$db_host --db_port=$db_port --db_user=$MYSQL_CREDS_USR --db_pass=$MYSQL_CREDS_PSW &'
                        }
                    } catch (Exception e) {
