...
pipeline {
    ...
    stages {
        ...
        stage('Run Backend Testing') {
         ...
         stage('Run Clean Up - Backend') {
            steps {
                echo 'Running Clean Up - Backend'
                script {
                    try {
                        if (checkOs() == 'Windows') {
                        ...
                        echo 'Exception Running Clean Up - Backend!'
                        error('Aborting the build')
                    }
                }
            }
        ...
        stage('Run Clean Up - Docker-Compose') {
            steps {
                echo 'Running Cleanup - Docker-Compose'
                script {
                    try {
                        if (checkOs() == 'Windows') {
        ...
        stage('Write service URL') {
            steps {
                echo 'Write service URL'
                script {
                    try {
                        if (checkOs() == 'Windows') {
                            bat 'minikube service devops-rest-service –url > k8s_url.txt'
                        } else {
                            sh 'minikube service devops-rest-service –url > k8s_url.txt'
                        }
                    } catch (Exception e) {
                        echo 'Exception Running Write service URL!'
                        error('Aborting the build')
                    }
                }
            }
        ...
        stage('Helm test') {
            steps {
                ...
                script {
                    try {
                        if (checkOs() == 'Windows') {
                            bat 'python K8S_backend_testing.py $remote_db_host $remote_db_port $REMOTE_MYSQL_CREDS_USR $REMOTE_MYSQL_CREDS_PSW'
                        } else {
                            sh '${python_run_file} K8S_backend_testing.py $remote_db_host $remote_db_port $REMOTE_MYSQL_CREDS_USR $REMOTE_MYSQL_CREDS_PSW'
                        }
                    } catch (Exception e) {
                        echo 'Exception Running Helm test!'
                        error('Aborting the build')
                    }
                }
            }
        }
        stage('Run Clean Up - Helm') {
            steps {
                echo 'Running Cleanup - Helm'
                script {
                    try {
                        if (checkOs() == 'Windows') {
                            ...
                            sh 'helm delete devops'
                            sh 'rm k8s_url.txt'
                        }
                    } catch (Exception e) {
                        echo 'Exception Running Cleanup - Helm!'
                        error('Aborting the build')
                    }
                }
            }
        ...
        always {
            ...
            script {
                ...
                try {
                    ...
                    echo 'Exception Running Post Cleanup! {e}'
                }
            }
        }
        failure {
